...
# below to the results you observed.
set -euo pipefail
...
# The operator edits operator_run by hand (this script cannot observe a real R2
# run), so a naive rewrite would clobber it. We rewrite only the sha and date
# lines in place.
tmp="$(mktemp)"

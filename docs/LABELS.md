...
| `verify.expect` | string | unset | Optional check on the probe's stdout. A bare value is a **literal substring** the stdout must contain; prefix with `re:` to match as a **regular expression** instead (e.g. `re:^[1-9][0-9]*$` for a row-count assertion). A bare value is never interpreted as a regex, so metacharacters in a literal expect (a version like `1.0`) match literally rather than silently broadening the match into a false pass. An explicit `re:` pattern that does not compile matches nothing (fail closed). |
| `verify.timeout` | Go duration | `10m` | Wall clock for the whole verify, including image pull and container start. `ballast verify --timeout <dur>` overrides it for a single invocation. |
